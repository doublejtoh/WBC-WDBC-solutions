...
wbc = np.loadtxt('Data/breast-cancer-wisconsin.txt',delimiter=',',dtype=np.float32)
training_data_x = wbc[100:,1:-1] # training_data set X
...
print('--------------------------')
test_data_x = wbc[-100:,1:-1] # test_data set_x
...
with tf.variable_scope('layer1') as scope:
    W1 = tf.get_variable("W", shape=[9, 8],
...
with tf.variable_scope('layer2') as scope:
    W2 = tf.get_variable("W", shape=[8, 8],
                         initializer=tf.contrib.layers.xavier_initializer())
...
with tf.variable_scope('layer3') as scope:
    W3 = tf.get_variable("W", shape=[8, 8],
                         initializer=tf.contrib.layers.xavier_initializer())
...
with tf.variable_scope('layer4') as scope:
    W4 = tf.get_variable("W", shape=[8, 8],
                         initializer=tf.contrib.layers.xavier_initializer())
...
with tf.variable_scope('layer5') as scope:
    W5 = tf.get_variable("W", shape=[8, 2],
                         initializer=tf.contrib.layers.xavier_initializer())
...
writer = tf.summary.FileWriter('./logs/wbc_project_ds1')
writer.add_graph(sess.graph)
global_step = 0
print('Start learning!')
# train my model
print(training_data_x[0:100].shape)
